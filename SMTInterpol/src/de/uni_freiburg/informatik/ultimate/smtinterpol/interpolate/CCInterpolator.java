...
public class CCInterpolator {
	 ...
	 */
	private Term[] interpolateCongruence(final ApplicationTerm left, final ApplicationTerm right, final Term clause) {
		...
		final String source = mInterpolator.getClauseTermInfo(clause).getSource();
	...
	public Term[] computeInterpolants(final Term proofTerm) {
		...
		if (mPath.length == 2) {
			return interpolateCongruence((ApplicationTerm) mPath[0], (ApplicationTerm) mPath[1], proofTerm);
		} else {
